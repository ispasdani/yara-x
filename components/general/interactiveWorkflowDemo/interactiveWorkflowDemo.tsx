    ...
    <Card className={`min-w-[200px] ${selected ? "ring-2 ring-primary" : ""}`}>
      ...
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          ...
          {(data as NodeData).label || "Upload Contract"}
        </CardTitle>
      </CardHeader>
      <CardContent className="pb-3">
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">
            Upload rental agreement and related documents
          </div>
...
const InteractiveWorkflowDemo = () => {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      setExecutionSteps(prev => prev.map(s => 
        s.nodeId === step.nodeId 
          ? { ...s, status: 'running', startTime: new Date().toLocaleTimeString() }
          : s
      ));
      ...
      setExecutionSteps(prev => prev.map(s => 
        s.nodeId === step.nodeId 
          ? { 
              ...
              result: fakeResults[step.nodeId] || 'Processing completed successfully'
            }
          : s
      ));
  ...
  const nodeCategories = [
    {
      ...
      items: [
        { id: 'contractUpload', name: 'Upload Contract', icon: Upload, description: 'Upload rental agreement' },
        { id: 'contractAnalysis', name: 'Analyze Contract', icon: Search, description: 'Extract contract terms' }
      ]
    },
    {
      name: "Legal Research",
      items: [
        { id: 'tenantRights', name: 'Research Rights', icon: Globe, description: 'Research tenant protections' },
        { id: 'disputeClassification', name: 'Classify Dispute', icon: CheckCircle, description: 'Categorize the dispute' }
      ]
    },
    {
      name: "Strategy",
      items: [
        { id: 'strategyRecommendation', name: 'Legal Strategy', icon: PenTool, description: 'Generate legal recommendations' }
      ]
    }
  ];
  ...
  return (
    <div className="relative bg-gradient-to-br from-surface to-background rounded-2xl border border-border shadow-lg overflow-hidden h-[700px]">
      ...
      >
        {leftPanelVisible ? <ChevronLeft className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
      </Button>
            ...
            <div className="flex-1 p-4">
              ...
              <ScrollArea className="h-full">
                {executionSteps.length === 0 ? (
                  <p className="text-xs text-muted-foreground">Click &quot;Run Workflow&quot; to see execution steps</p>
                ) : (
                  <div className="space-y-2">
                    {executionSteps.map(step => (
                      <div key={step.id} className="border border-border rounded-lg p-2">
                        <div 
                        ...
                        {step.expanded && (
                          <div className="mt-2 pl-5 text-xs text-muted-foreground space-y-1">
                            {step.startTime && (
                              <p>Started: {step.startTime}</p>
                            )}
                            {step.endTime && (
                              <p>Completed: {step.endTime}</p>
                            )}
                            {step.result && (
      ...
      >
        {rightPanelVisible ? <ChevronRight className="w-3 h-3" /> : <ChevronLeft className="w-3 h-3" />}
      </Button>
    </div>
  );
};
